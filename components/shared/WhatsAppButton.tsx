...
export function WhatsAppButton({ telefono, nombre, fecha, numeroCuota, monto }: Props) {
  const telefonoLimpio = telefono.replace(/\D/g, "");

  if (!telefonoLimpio) {
    return (
      <span
        title="Agregar teléfono al contrato para enviar recordatorio"
        className="cursor-help inline-flex items-center"
      >
        <WAIcon style={{ opacity: 0.3 }} />
      </span>
    );
  }
